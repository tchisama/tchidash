...
import { collection, orderBy, query, where, limit, startAfter, getAggregateFromServer, count, sum } from "firebase/firestore";
...
import { DollarSign, MapPin, MapPinCheck, PhoneIcon, ShoppingBasket, X } from "lucide-react";
...
export default function CustomerPage() {
  ...
  const { data: customers = [], isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      setLastDoc(response.docs.length ? response.docs[response.docs.length - 1] : null);
  ...
  return (
    <div>
      ...
      <div className="flex justify-center mt-6 space-x-4">
        ...
        <Button
          variant="outline"
          onClick={() => setPage((prev) => (customers.length < ITEMS_PER_PAGE ? prev : prev + 1))}
...
export function CustomerCardByNumber({number: phoneNumber}: {number: string}) {
  const { store } =useStore();
  const { data } = useQuery({
    queryKey: ["customer", phoneNumber],
    queryFn: async () => {
      if (!store) return null;
      const q = query(collection(db, "customers"), where("phoneNumber", "==", phoneNumber), where("storeId", "==", store.id));
      const response = await dbGetDocs(q, store.id, "");
      return response.docs.map((doc) => ({ ...doc.data(), id: doc.id }) as Customer)[0];
    },
  });
    ...
    const {data} = useQuery({
      ...
      queryFn: async () => {
        ...
        const q3 = query(
          ...
          where("customer.phoneNumber", "==", customer.phoneNumber),
          where("storeId", "==", customer.storeId),
          where("orderStatus", "in",["returned","cancelled","fake","no_reply"])
        )
      ...
      <div className="flex w-full items-center justify-between space-x-6 p-6">
        ...
        <div className="flex-1 truncate">
          <div className="flex items-center space-x-3">
            <h3 className="truncate text-sm font-medium text-gray-900">{customer.firstName} {customer.lastName} </h3>
          </div>
          {customer.phoneNumber && <p className="mt-1 truncate text-sm text-gray-500">{customer.phoneNumber}</p>}
        </div>
        <div>
          {data && (
            <div className="">
          ...
          <div className="px-3 p-2">
                <p className="mt-1 truncate  text-gray-700 flex items-center gap-1"><MapPin className="h-4 w-4 inline-block" /> {customer?.address?.city}</p>
                <p className="mt-1 truncate text-sm text-gray-500">{customer?.address?.street}</p>
          </div>
      <div>
        <div className="-mt-px flex divide-x divide-gray-200">
          <div className="flex w-0 flex-1">
            <a
              ...
              <IconBrandWhatsapp aria-hidden="true" className="h-5 w-5 text-gray-400" />
              Open Chat
            </a>
          </div>
