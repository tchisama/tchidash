...
export function VariantSelectorElement({
  element,
}: VariantSelectorElementProps) {
    ...
    return (
      <div style={containerStyle}>
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Please select a product to display variants
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  if (
    ...
    return (
      <div style={containerStyle}>
        <Alert>
          <AlertDescription>
            This product has no variants to display
          </AlertDescription>
        </Alert>
      </div>
    );
      ...
      {selectedProduct.options.map((option, index) => (
        <div key={index} className="mb-6 last:mb-0">
          ...
          <div style={titleStyle}>
            Select {content.customOptionName || option.name}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {selectedProduct.variants.map((variant) => (
                <div key={variant.id} className="relative">
                  ...
                  <Label
                    htmlFor={variant.id}
                    className="flex flex-col items-center justify-between rounded-md border-2 border-muted bg-white p-1 hover:bg-gray-50 hover:border-gray-300 peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary"
                  >
                    ...
                    {
                      // if all variants have the same price, dont show it
                      selectedProduct.variants.every(
                        (v) => v.price === variant.price,
                      ) ? (
                        <div className="py-1"></div>
                      ) : (
                        // Show price only if it's different from the others
        ...
        <div className="mt-6 space-y-4">
          <div className="p-4 bg-white rounded-md border">
            ...
            <div className="text-sm text-muted-foreground mb-4">
              Unit Price: {selectedVariant.price} Dh
            </div>
            ...
            <div className="flex items-center space-x-2 mb-4">
              ...
              <div className="flex items-center">
                ...
                <Button
                  ...
                  onClick={() => handleQuantityChange(quantity + 1)}
                  disabled={
                    quantity >= (selectedVariant.inventoryQuantity || 100)
                  }
                >
            ...
            <div className="text-lg font-bold">Total: {getTotalPrice()} Dh</div>
          </div>
        </div>
      )}
    </div>
