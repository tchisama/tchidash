...
interface ReviewsStyleFormProps {
  element: PageElement
  onUpdate: (updates: Partial<PageElement>) => void
}

export function ReviewsStyleForm({ element, onUpdate }: ReviewsStyleFormProps) {
  const { style } = element
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border-t pt-4 mt-4">
        <h3 className="font-medium mb-3">Review Card Styling</h3>

        <div className="space-y-2">
          <Label>Review Background Color</Label>
          <div className="flex gap-2">
            <Input
              type="color"
              value={style.reviewBgColor || "#f9fafb"}
              onChange={(e) => updateStyle("reviewBgColor", e.target.value)}
              className="w-12 h-10 p-1"
            />
            <Input
              type="text"
        ...
        <div className="space-y-2">
          <Label>Review Border Radius (px)</Label>
          <Slider
            value={[style.reviewBorderRadius || 8]}
            min={0}
            max={20}
            step={1}
            onValueChange={(value) => updateStyle("reviewBorderRadius", value[0])}
          />
          <div className="text-right text-sm text-muted-foreground">{style.reviewBorderRadius || 8}px</div>
        </div>

        <div className="space-y-2">
          <Label>Review Padding (px)</Label>
          <Slider
            value={[style.reviewPadding || 16]}
            min={8}
            max={32}
            step={1}
            onValueChange={(value) => updateStyle("reviewPadding", value[0])}
          />
          <div className="text-right text-sm text-muted-foreground">{style.reviewPadding || 16}px</div>
        </div>

        <div className="space-y-2">
          <Label>Space Between Reviews (px)</Label>
          <Slider
            value={[style.reviewSpacing || 16]}
            min={8}
            max={32}
            step={1}
            onValueChange={(value) => updateStyle("reviewSpacing", value[0])}
          />
          <div className="text-right text-sm text-muted-foreground">{style.reviewSpacing || 16}px</div>
        </div>
      </div>
