...
export function SpecificationsElement({ element }: SpecificationsElementProps) {
  ...
  return (
    <div style={containerStyle}>
      <h2 style={titleStyle}>{content.sectionTitle || "Product Specifications"}</h2>
      <p style={subtitleStyle}>{content.subtitle || "Technical details and dimensions"}</p>
          ...
          <tbody>
            {specifications.map((spec: Specification, index: number) => (
              <tr
                key={spec.id}
                style={{
                  backgroundColor: index % 2 === 0 ? style.rowBgColor || "#f9fafb" : style.altRowBgColor || "#ffffff",
                }}
              >
