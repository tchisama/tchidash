...
import { dbAddDoc, dbGetDoc, dbGetDocs } from '@/lib/dbFuntions/fbFuns';
...
function OrderNotes() {
  ...
  return (
    <Card className="flex  relative border-none  flex-col flex-1 ">
      <CardContent className="px-4 overflow-y-auto border-t  py-6 space-y-4 flex-1">
        {error?.message}
        {
          notes?.map((note)=>{
      ...
      <CardFooter className="flex flex-col   bottom-0 w-full left-0 mt-auto">
        <form onSubmit={onSubmit}
         className="flex w-full items-start space-x-2 pt-4 border-t">
          <Textarea
            value={content}
      ...
      <div className='flex-1 items-start justify-start'>
        <div className='flex justify-between'>
          ...
          <div className='text-sm text-gray-500 flex gap-2'>
            ...
            <span>{new Date(note.createdAt.toDate()).toLocaleTimeString()}</span>
          </div>
        </div>
    <div
