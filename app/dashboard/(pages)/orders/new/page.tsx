...
import { addDoc, and, collection, getDocs, query, Timestamp, where } from 'firebase/firestore'
import { useStore } from '@/store/storeInfos'
import { db } from '@/firebase'
import { useQuery } from '@tanstack/react-query'
import { CustomerInfo, Order, ShippingInfo } from '@/types/order'
...
export default function CreateOrder() {
  ...
  const { data:products} = useQuery({queryKey:["products",storeId],queryFn:()=>{
    const q = query(
    ...
    const response = getDocs(q).then(response => response.docs.map((doc) => ({...doc.data(), id: doc.id} as Product)))
    return response
  }})
  ...
  const handleProductChange = (value: string) => {
    ...
    totalPrice: (products?.find(p => p.id === value)?.price || 0) * (prev.quantity || 1),
    title: products?.find(p => p.id === value)?.title || '',
    }))
  }
  ...
  const handleVariantChange = (value: string) => {
    ...
    setCurrentItem(prev => ({ ...prev, variantId: value, 
    price: variant ? variant.price : 0,
    totalPrice: (variant ? variant.price : 0) * (prev.quantity || 1),
    title: products?.find(p => p.id === prev.productId)?.title + " [" + (variant ? variant.title +"]": ''),
    }))
  }

  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setCurrentItem(prev => ({ ...prev, quantity: parseInt(e.target.value) || 1,
      totalPrice:prev.price * parseInt(e.target.value)
    }))
  }
...
const handleSubmit = (e: React.FormEvent) => {
  ...
  const { totalItems, subtotal } = calculateTotals(orderItems);
  ...
  const shippingInfo: ShippingInfo = {
    ...
    cost: 5.00,          // Default shipping cost or retrieve dynamically
    ...
    <Card className="w-full mx-auto">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid max-w-3xl grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="firstName">First Name</Label>
              <Input id="firstName" name="firstName" value={order.firstName} onChange={handleChange} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="lastName">Last Name</Label>
              <Input id="lastName" name="lastName" value={order.lastName} onChange={handleChange} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="phoneNumber">Phone Number</Label>
              <Input id="phoneNumber" name="phoneNumber" value={order.phoneNumber} onChange={handleChange} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="address">Address</Label>
              <Input id="address" name="address" value={order.address} onChange={handleChange} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="city">City</Label>
              <Input id="city" name="city" value={order.city} onChange={handleChange} required />
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={currentItem.variantId} onValueChange={handleVariantChange}>
                  ...
                  <SelectContent>
                    {currentItem.productId && variants && variants.length > 0 && variants.map(variant => (
          ...
          <Table>
            ...
            <TableBody>
              ...
              orderItems.map((item, index) => (
                <TableRow key={index}>
                  <TableCell>{ products.find(p => p.id === item.productId)?.title}</TableCell>
                  <TableCell>{ variants.find(v => v.id === item.variantId)?.title}</TableCell>
