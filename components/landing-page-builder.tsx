...
export function LandingPageBuilder() {
  ...
  const addSection = (sectionType: string, sectionName: string, elements: Element[] = []) => {
  ...
  const addElement = (sectionId: string, elementType: string, content: any = {}, props: any = {}) => {
    ...
    setSections(
      sections.map((section) =>
        section.id === sectionId ? { ...section, elements: [...section.elements, newElement] } : section,
      ),
    )
  ...
  const updateElement = (sectionId: string, elementId: string, updates: Partial<Element>) => {
    setSections(
      sections.map((section) =>
  ...
  const moveElement = (sectionId: string, fromIndex: number, toIndex: number) => {
    setSections(
      sections.map((section) => {
  ...
  const selectedElement = selectedSection?.elements.find((element) => element.id === selectedElementId)
